...
def normalize_cluster_state(state):
    if isinstance(state, dict):
        if 'clusterNodes' in state and 'nodes' not in state:
            state['nodes'] = state['clusterNodes']
        elif 'nodes' in state and 'clusterNodes' not in state:
            state['clusterNodes'] = state['nodes']
    return state


def self_node(state):
    nodes = state.get('nodes') or state.get('clusterNodes') or []
    for node in nodes:
...
def run(client):
    ...
    state = normalize_cluster_state(client.call('/api/admin/cluster/state', method='GET'))

    if params['state'] == 'absent':
        res = ensure_absent(client, state, module.check_mode)
    elif params['role'] == 'primary':
        res = ensure_primary(client, state, module.check_mode)
    else:
        res = ensure_secondary(client, state, module.check_mode)

    if 'cluster' in res:
        res['cluster'] = normalize_cluster_state(res['cluster'])
    return res
